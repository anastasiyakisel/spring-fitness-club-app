...
public class DAOJdbcRegistration extends DAOJdbc implements IDAORegistration{
    ...
    private static final String DELETE_FROM_GROUP_IN_REGISTRATION_STATEMENT=
            "DELETE FROM `registration` WHERE person_id=(?) AND group_id=(?)";
    private static final String COUNT_CLIENTS_IN_GROUP_STATEMENT=
            "SELECT COUNT(*) FROM `registration` WHERE group_id=(?)";
    private static final String SELECT_USER_GROUPS_STATEMENT=
            "SELECT group_id FROM `registration` WHERE person_id=(?)";
    private static final String SELECT_ALL_USER_GROUPS_STATEMENT=
            "SELECT * FROM groups gr INNER JOIN sporttype sp ON gr.sporttype_id=sp.sporttype_id WHERE gr.group_id=(?)";
    private static final String COUNT_NUMBER_OF_ABONEMENTS_STATEMENT=
            "SELECT COUNT(*) FROM `registration` WHERE person_id=(?)";
    private static final String INSERT_USER_INTO_REGISTRATION_STATEMENT=
            "INSERT INTO `registration`(person_id, group_id) VALUES (?, ?) ";
    private static final String SELECT_USERS_FROM_GROUPS_STATEMENT = "SELECT * FROM person p JOIN registration r ON"
	...
	PreparedStatement statement = null;
        ...
        try {
            statement = connection.prepareStatement(DELETE_FROM_GROUP_IN_REGISTRATION_STATEMENT);
	...
	PreparedStatement statement =null;
        ...
        int count=0;
        try {
            statement = connection.prepareStatement(COUNT_CLIENTS_IN_GROUP_STATEMENT);
    ...
    public ArrayList<Integer> showUserGroupIds(int userId) 
        ...
        connection=connectionPool.getConnection(MAX_CONNECTION_WAIT);
        try {
            st = connection.prepareStatement(SELECT_USER_GROUPS_STATEMENT);
            st.setInt(1, userId);
    ...
    public ArrayList<Group> showUserGroups(int personId) throws DAOSQLException, 
        ...
        connection = connectionPool.getConnection(MAX_CONNECTION_WAIT);
        try {
            statement = connection.prepareStatement(SELECT_USER_GROUPS_STATEMENT);
            ...
            for (int groupId:groupIds){
                statement=connection.prepareStatement(SELECT_ALL_USER_GROUPS_STATEMENT);
	...
	public int countNumberOfAbonementsForUser(int personId)
        ...
        try {
            connection = connectionPool.getConnection(MAX_CONNECTION_WAIT);
            st=connection.prepareStatement(COUNT_NUMBER_OF_ABONEMENTS_STATEMENT);
	...
	public boolean addUserToGroups(User entity, Integer... recIds)
        ...
        try {
            connection=connectionPool.getConnection(MAX_CONNECTION_WAIT);
            statement=connection.prepareStatement(INSERT_USER_INTO_REGISTRATION_STATEMENT);
            for (Integer recId : recIds){
	...
	public ArrayList<User> getUsersFromGroup(int groupNumber)
        ...
        connection=connectionPool.getConnection(MAX_CONNECTION_WAIT);
        try {
            statement=connection.prepareStatement(SELECT_USERS_FROM_GROUPS_STATEMENT);
            statement.setInt(1, groupNumber);
            ResultSet rs=statement.executeQuery();
            while(rs.next()){
                User user = new User();
